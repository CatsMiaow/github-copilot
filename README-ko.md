...
- [copilot-agent.instructions.md](.github/instructions/copilot-agent.instructions.md): 단일 진실 공급원(SSOT) — 매 응답마다 3가지 필수 동작(순차적 사고, 스킬 게이트, 서브에이전트 위임)을 강제하며 5단계 실행 프로토콜과 금지 호출 규칙을 포함
- [subagent-templates.instructions.md](.github/instructions/subagent-templates.instructions.md): 서브에이전트 참조 문서 — Context Package 명세, 순차/병렬 실행 결정 가이드, 리서치 및 구현 서브에이전트용 프롬프트 템플릿
